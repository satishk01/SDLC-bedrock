...
def expand_requirement(basic_requirement: str, requirement_type: str, bedrock_client) -> str:
    ...
    log_workflow_step("expand_requirement", "started", {
    ...
    # Log the request
    log_llm_request(prompt, "requirement_expansion", MODEL_ID)
    ...
    try:
        ...
        log_llm_response(detailed_requirement,
                         "requirement_expansion", MODEL_ID, processing_time)
        ...
        log_llm_error(str(e), "requirement_expansion", MODEL_ID)
...
def breakdown_requirement(detailed_requirement: str, requirement_type: str, bedrock_client) -> List[str]:
    ...
    log_workflow_step("breakdown_requirement", "started",
                      {"requirement_type": requirement_type, "requirement_length": len(detailed_requirement)})

    prompt = f"""You are a business analyst. Break down the following detailed {requirement_type} requirement into specific, actionable tasks.
    
    Detailed Requirement:
    {detailed_requirement}
    
    Please provide the breakdown as a JSON array where each element is a string describing a specific task.
    The tasks should be:
    - Specific and actionable
    - Independent of each other
    - Suitable for creating individual JIRA stories
    - Focused on deliverable outcomes
    
    Format your response as a valid JSON array only, like:
    ["Task 1 description", "Task 2 description", "Task 3 description"]"""

    # Log the request
    log_llm_request(prompt, "requirement_breakdown", MODEL_ID)
    start_time = time.time()
    ...
    try:
        ...
        # Log successful response
        processing_time = time.time() - start_time
        log_llm_response(content, "requirement_breakdown",
                         MODEL_ID, processing_time)
        ...
        try:
            ...
            if isinstance(tasks, list):
                ...
                log_workflow_step("breakdown_requirement", "failed",
                                  {"requirement_type": requirement_type, "error": "Response is not a list"})
                return []
            ...
            if json_match:
                try:
                    ...
                    if isinstance(tasks, list):
                        ...
                        log_workflow_step("breakdown_requirement", "failed",
                                          {"requirement_type": requirement_type, "error": "Extracted content is not a list"})
                        return []
                except json.JSONDecodeError:
                    log_workflow_step("breakdown_requirement", "failed",
                                      {"requirement_type": requirement_type, "error": "JSON parsing failed after extraction"})
                    pass
            log_workflow_step("breakdown_requirement", "failed",
                              {"requirement_type": requirement_type, "error": "Could not extract valid JSON"})
            st.error("Could not find valid task list in Claude's response")
            return []

    except Exception as e:
        log_llm_error(str(e), "requirement_breakdown", MODEL_ID)
...
def create_jira_story(requirement: str, requirement_type: str, bedrock_client) -> Dict:
    ...
    # Log the request
    log_llm_request(prompt, "jira_story_creation", MODEL_ID)
    start_time = time.time()
    ...
    try:
        ...
        # Log successful response
        processing_time = time.time() - start_time
        log_llm_response(content, "jira_story_creation",
                         MODEL_ID, processing_time)
        ...
        if start_idx != -1 and end_idx != -1:
            ...
            log_workflow_step("create_jira_story", "completed",
                              {"story_summary": story.get('Summary', 'Unknown'),
        ...
        log_llm_error(str(e), "jira_story_creation", MODEL_ID)
        log_workflow_step("create_jira_story", "error", {"error": str(e)})
...
def main():
        ...
        if basic_requirement:
            ...
            with st.spinner("Expanding requirement..."):
                ...
                if detailed_requirement:
                    ...
                    log_workflow_step("main_workflow", "expand_completed", {
                                      "requirement_type": requirement_type})
                    st.success("Requirement expanded successfully!")
                else:
                    log_workflow_step("main_workflow", "expand_failed", {
                                      "requirement_type": requirement_type})
    ...
    if st.session_state.detailed_requirement:
        ...
        if st.button("Break Down into Tasks"):
            log_workflow_step("main_workflow", "breakdown_started", {
                              "requirement_type": requirement_type})
            with st.spinner("Breaking down requirement into tasks..."):
                tasks = breakdown_requirement(
                    st.session_state.detailed_requirement, requirement_type, bedrock_client)
                if tasks:
                    log_workflow_step("main_workflow", "breakdown_completed",
                                      {"requirement_type": requirement_type, "task_count": len(tasks)})
                    st.success(
                        f"Requirement broken down into {len(tasks)} tasks!")

                    # Create JIRA story for each task
                    log_workflow_step("main_workflow", "story_creation_started",
                                      {"requirement_type": requirement_type, "task_count": len(tasks)})
                    successful_stories = 0
                    for i, task in enumerate(tasks, 1):
                        story = create_jira_story(
                            task, requirement_type, bedrock_client)
                        if story:
                            st.session_state.stories.append(story)
                            successful_stories += 1

                    log_workflow_step("main_workflow", "story_creation_completed",
                                      {"requirement_type": requirement_type, "total_tasks": len(tasks),
                                       "successful_stories": successful_stories,
                                       "success_rate": successful_stories/len(tasks) if tasks else 0})
                else:
                    log_workflow_step("main_workflow", "breakdown_failed", {
                                      "requirement_type": requirement_type})
    ...
    if st.session_state.stories:
        ...
        pdf_data = create_pdf_report(
            st.session_state.stories, st.session_state.detailed_requirement)
        with col2:
            st.download_button(
                label="Download PDF",
                data=pdf_data,
